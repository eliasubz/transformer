...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head):
        # n_embd: embedding deimension, n_head: the number of heads we'd like
        super().__init__()
...
class MyLinear(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias=True):
        ...
        self.weight = nn.Parameter(
            (torch.randn(out_features, in_features) - 0.5) * (1.0 / in_features**0.5)
        )
        if bias:
    ...
    def forward(self, x):
        ...
        if T != self.in_features:
            print("Wrong input. Expected: ", B, self.in_features, "\ngot: ", B, T)
            return KeyError
...
# my_x = torch.randn((3, 5))
# myLinear = MyLinear(4, 6)
# print(my_x)
# my_x = myLinear(my_x)
# print(my_x)
# print(12 / 0)
